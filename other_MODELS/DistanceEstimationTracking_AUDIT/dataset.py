...
def pcd_to_sparsetensor_custom(pcd, mask_valid, voxel_size=0.01, num_points=100000):
    # Safe default from pcd itself: finite XYZ and positive depth.
    pcd = np.asarray(pcd)
    finite_mask = np.isfinite(pcd[..., 0]) & np.isfinite(pcd[..., 1]) & np.isfinite(pcd[..., 2])
    valid_mask = finite_mask & (pcd[..., 2] > 0)

    # Use external mask only when clearly well-formed; otherwise keep safe default.
    if isinstance(mask_valid, tuple) and len(mask_valid) >= 2:
        r_idx = np.asarray(mask_valid[0]).reshape(-1)
        c_idx = np.asarray(mask_valid[1]).reshape(-1)

        # Keep only finite integer-like indices.
        if r_idx.size > 0 and c_idx.size > 0:
            n = min(r_idx.size, c_idx.size)
            r_idx = r_idx[:n]
            c_idx = c_idx[:n]
            pair_finite = np.isfinite(r_idx) & np.isfinite(c_idx)
            r_idx = r_idx[pair_finite].astype(np.int64, copy=False)
            c_idx = c_idx[pair_finite].astype(np.int64, copy=False)

            # Choose orientation that yields more in-bounds pairs.
            in_rc = (r_idx >= 0) & (r_idx < pcd.shape[0]) & (c_idx >= 0) & (c_idx < pcd.shape[1])
            in_cr = (c_idx >= 0) & (c_idx < pcd.shape[0]) & (r_idx >= 0) & (r_idx < pcd.shape[1])
            if np.count_nonzero(in_cr) > np.count_nonzero(in_rc):
                r_idx, c_idx = c_idx, r_idx
                in_rc = in_cr

            if np.any(in_rc):
                r_idx = r_idx[in_rc]
                c_idx = c_idx[in_rc]
                tuple_mask = np.zeros(pcd.shape[:2], dtype=bool)
                tuple_mask[r_idx, c_idx] = True
                valid_mask = valid_mask & tuple_mask
    else:
        mask_arr = np.asarray(mask_valid)
        if mask_arr.dtype == np.bool_:
            if mask_arr.shape == pcd.shape[:2]:
                valid_mask = valid_mask & mask_arr
            elif mask_arr.shape == pcd.shape[:2][::-1]:
                valid_mask = valid_mask & mask_arr.T
            # Any other shape is ignored to avoid unsafe indexing.

    pcd_valid = pcd[valid_mask]
    if pcd_valid.size == 0:
        # Keep downstream code alive for degenerate frames.
        pcd_valid = np.zeros((1, 3), dtype=pcd.dtype)
