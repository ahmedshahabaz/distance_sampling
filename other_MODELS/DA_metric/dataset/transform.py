...
class CustomAugmentation:
    def __init__(self, size=(518, 518)):
        self.size = size

    def __call__(self, img, mask, gt_mean):
        # Random Horizontal Flip
        if random.random() < 0.5:
            img = TF.hflip(img)
            mask = TF.hflip(mask)
            gt_mean = TF.hflip(gt_mean)

        # Random Vertical Flip
        if random.random() < 0.5:
            img = TF.vflip(img)
            mask = TF.vflip(mask)
            gt_mean = TF.vflip(gt_mean)
        ...
        if random.random() < 0.5:
            ...
            gt_mean = TF.rotate(gt_mean, angle, expand=False)

        # Random Color Jitter (only on the image)
        if random.random() < 0.5:
            img = TF.adjust_brightness(img, random.uniform(0.8, 1.2))
            img = TF.adjust_contrast(img, random.uniform(0.8, 1.2))
            img = TF.adjust_saturation(img, random.uniform(0.8, 1.2))

        return img, mask, gt_mean
