...
class PatchTransformerEncoder(nn.Module):
    def __init__(self, in_channels, patch_size=10, embedding_dim=128, num_heads=4):
        ...
        self.positional_encodings = nn.Parameter(torch.rand(4096, embedding_dim), requires_grad=True)
    ...
    def forward(self, x):
        ...
        pos_enc = self.positional_encodings.T.unsqueeze(0)  # (1, E, L)
        pos_enc = F.interpolate(pos_enc, size=embeddings.shape[2], mode='linear', align_corners=False)
